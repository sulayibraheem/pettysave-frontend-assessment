...
const actions = {
  ...
  async getPosts({ commit }) {
    ...
    try {
      ...
      const firstDisplay = await apiPosts.data.slice(0, 9);
  ...
  async getImages({ commit }) {
    try {
      const apiImages = await axios.get(
      ...
      commit("setAllImages", [...response, ...res, ...response]);
      commit("setStoreImages", [...response, ...res, ...response]);
    } catch (e) {
      console.error(e);
    } finally {
      commit("setShowSpinner", false);
  ...
  async getComments({ commit }) {
    try {
      const apiComments = await axios.get(
  ...
  async paginate({ commit, rootState }, { currentPage, perPage, posts }) {
    const start = (currentPage - 1) * perPage;
    const display = posts.slice(start, start + 9);
    if (posts == rootState.FavPost.favPost) {
  ...
  async search({ commit, dispatch }, { text }) {
    ...
    const updatePage = state.allPosts.filter((val) =>
